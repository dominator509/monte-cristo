...
fn tmp_cfg_dir() -> PathBuf {
    let d = std::env::temp_dir().join("mc_test_advisory");
...
fn advisory_shown_on_first_run() {
    ...
    assert!(!config.advisory_acknowledged, "fresh config must not acknowledge advisory");
...
fn advisory_acknowledged_persists() {
    ...
    assert!(loaded.advisory_acknowledged, "acknowledgement must persist after save");
    let _ = std::fs::remove_dir_all(&dir);
}
