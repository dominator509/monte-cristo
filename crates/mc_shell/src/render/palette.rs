...
pub fn sky_gradient(act: Act) -> Vec<(u16, u8, u16)> {
    match act {
        ...
        Act::ActIIIf => vec![(0, 1, 56), (56, 2, 56), (112, 3, 56), (168, 4, 56)],
        Act::ActIIIMonteCristo => vec![(0, 1, 56), (56, 2, 56), (112, 1, 56), (168, 2, 56)],
        Act::ActIVRome => vec![(0, 1, 56), (56, 3, 56), (112, 1, 56), (168, 3, 56)],
        Act::ActVParis => vec![(0, 1, 56), (56, 2, 56), (112, 1, 56), (168, 2, 56)],
        Act::ActVIParis => vec![(0, 5, 56), (56, 3, 56), (112, 5, 56), (168, 3, 56)],
        Act::ActVIIFinal => vec![(0, 1, 56), (56, 2, 56), (112, 1, 56), (168, 2, 56)],
    }
}
...
mod tests {
    ...
    fn palette_independence() {
        ...
        for act in &acts {
            ...
            for (s, h) in scene.iter().zip(hc.iter()).take(256) {
                if s.0 != h.0 {
                    different = true;
                    break;
                }
            ...
            assert!(
                different,
                "High-contrast palette must differ from {act:?} palette"
            );
        }
    }
    ...
    fn palette_count() {
        ...
        for act in &acts {
            ...
            assert!(
                non_zero > 0,
                "Palette for {act:?} must have non-zero entries"
            );
        }
    }
    ...
    fn high_contrast_selected_independently() {
        // Verify that using the high-contrast palette differs from the default interface palette.
        // Index 0 (transparent/black) is the same in both; check index 2 (window bg) instead.
        let hc_pal = high_contrast_palette();
        let default_pal = interface_palette();
        assert_ne!(
            hc_pal[2].0, default_pal[2].0,
            "HC palette window-bg should differ from default"
        );
    }
}
